...
Пример изображения из отрытого датасета MIDV
![Изображение](images\htmlimage.jpg)
Как мы можем видеть, изображение соответствует необходимому ракурсу и является чистым, без артефактов, шумов и других искажений, которые могли бы повлиять на его качество или применимость.

Датасет состоит из 1000 видеоклипов с комментариями, 1000 отсканированных изображений, 1000 фотографий, 1000 уникальных макетов документов, удостоверяющих личность, каждый с уникальными значениями текстовых полей и уникальными искусственно сгенерированными лицами.
...
Подготовить рабочий прототип бенчмарк-системы для задачи распознавания и классификации документов, чтобы продемонстрировать основные возможности системы

#### 3.1. Способ оценки пилота
Функционал пилота:

* Реализованный итератор по датасету
* Реализованный оценщик метрик
* Реализованный планировщик бенчмарка
* Реестр для хранения Docker-образов

Способ оценки пилота:

1. Тестирование:
    * Загрузка небольшого тестового набора данных.
    * Использование бенчмарк-системы для автоматической оценки качества распознавания и классификации.
    * Сравнение результатов моделей.
2. Критерии успеха пилота:
    * Система успешно обрабатывает тестовый набор данных и генерирует корректные метрики.
    * Полученные результаты для бейзлайн-модели соотносятся с ожидаемыми значениями.

#### 3.2. Что считаем успешным пилотом
Успешным пилот считается при выполнении следующих критериев:
1. Функциональность системы. Все основные модули бенчмарк-системы работают корректно, включая загрузку данных, выполнение предсказаний моделей и расчет метрик
2. Качество работы. Классификация документов и распознавание текста соответствуют заранее определенным диапазонам метрик.
3. Производительность. Время обработки тестового набора данных занимает заранее определенное количество времени.
#### 3.3. Подготовка пилота

В данном этапе я исследую возможности семейства VLM-моделей Qwen2-VL, а именно "Qwen2-VL-2B-Instruct".

Ключевые особенности моделей Qwen2-VL:
1. Открытая лицензия - Apache-2.0 (Qwen2-VL-2B, Qwen2-VL-7B).
2. SoTA в понимании изображений и видео ([Ссылка](https://github.com/QwenLM/Qwen2-VL#image-benchmarks), [Демо возможностей](https://qwenlm.github.io/blog/qwen2-vl/#model-capabilities)).
3. Старшая модель превосходит GPT4o, Claude 3.5 Sonnet по многим бенчмаркам ([Ссылка](https://qwenlm.github.io/blog/qwen2-vl/#performance)) 2B и 7B модели имеют почти такой же уровень точности на документах. Отлично подходят на роль локальной GPT4o.
    * Qwen2-VL-2B занимает 6 Гб GPU RAM
    * Qwen2-VL-7B занимает 17 Гб GPU RAM
4. Понимает длинный контекст, например многостраничный документ или видео 20 минут длинной.
5. Понимает много языков (может и понимать и общаться).
...
7. Регулируемый вход размера картинки.
    ...
    * По умолчанию принимает документ в исходном разрешении а не сжимает его как все модели. Благодаря этому видит мельчащие подробности в документе, которые не видят другие модели из за сжатия картинки на входе.
...
Вывод: идеальная точность.

![Паспорт](images/passport.jpg)
...
Вывод: почти идеальная точность.

![Диаграмма](images/Most-Popular-Social-Platforms-2021-1024x576.jpg)
...
![Архитектура](images\bl-sch.png)

### Компоненты и их функции

1. Benchmark Scheduler (Docker Container):
   - Планировщик бенчмарков считывает конфигурацию от пользователя и запускает все пайплайны бенчмарков (1 датасет = 1 модель).

2. VLLM run:
   - Скрипт для инициализации и запуска vLLM-модели на датасете.

3. VLLM model:
   - Класс, реализующий модель. Используется VLLM run для выполнения модели.

4. Dataset Runner:
   - Получает вопрос и изображение из датасета, получает ответ от модели, собирает все ответы и сохраняет ответы модели в CSV.

5. Dataset Iterator:
   - Реализует итератор по датасету, отдает один элемент из датасета.

6. Prompt Adapter:
   - Отдает оптимальные промпты по названию поля документа и модели.

7. System Prompt Adapter:
   - Отдает системный промпт модели для указанного датасета, если это указано.

### Взаимодействие компонентов

- Benchmark Scheduler запускает VLLM run, который инициализирует и запускает VLLM model.
- VLLM model использует System Prompt Adapter для получения системного промпта и Prompt Adapter для получения оптимальных промптов.
- Dataset Iterator предоставляет данные для Dataset Runner, который затем получает ответы от VLLM model и сохраняет их в CSV файл.
- System Prompt Adapter читает системные промпты из контейнера SystemPrompts, а Prompt Adapter читает коллекцию оптимальных промптов из контейнера PromptCollection.
- Dataset Iterator читает данные из контейнера Datasets, который содержит CSV файлы и папки с изображениями.
- Dataset Runner сохраняет ответы модели в контейнер ModelsAnswers, который хранит CSV файлы с ответами модели.

### Дополнительные контейнеры

- cfg: JSON файл с переменными и env с секретами.
- SystemPrompts: Папка с папками с одним .txt файлом, содержащим системные промпты.
- PromptCollection: Папка с CSV файлами с промптами для полей.
- Datasets: Папка с CSV файлами и папкой с изображениями.
- ModelsAnswers: Папка с CSV файлами с ответами модели.

Таким образом, данная система автоматизированно выполняет бенчмарки моделей на различных датасетах, используя определенные промпты и сохраняя результаты в удобном формате для дальнейшего анализа.

Запуск ПО состоит из 4 этапов:
* #### Планирование задач

    На первом этапе UserConfigReader считывает из csv-конфига от пользователя набор пробегов. Каждый пробег описывается классом BenchmarkRunConfig. Далее BenchmarkOrchestrator запускает пайплайн из докер контейнеров согласно параметрам пробега.

![Планирование](images\bench_or.png)

* #### Запуск ВЛМ в докер контейнерах

    Далее, VLM читает конфиг пробега, загружает датасет, получает коллекцию оптимальных промптов и системный промпт. Происходит загрузка модели, итерирование по датасету и сохранение ответов модели.
...
![Оценка метрик](images\metr_ev.png)

* #### Генерация отчета с диаграммами.

    После многократного анализа в Jupyter ноутбуке, мы поймем, какие отчеты являются наиболее важными. В планах разработать отдельный модуль, который будет создавать эти отчеты.

![Генерация отчета](images\reports.png)


### 4.2. Описание инфраструктуры и масштабируемости
Разрабатываемое программное обеспечение VLMHyperBench представляет собой open-source проект, доступный для использования специалистами IT-компаний, исследовательскими группами и не только, с возможностью его локального развертывания.

#### Инфраструктура
Основным языком разработки является Python (версия 3.10 и выше), а для управления зависимостями используется Poetry. Для вычислений необходимы собственные или облачные серверы с GPU либо GPU-кластеры с системой управления заданиями; при использовании нескольких GPU важно, чтобы они были одинаковыми для корректной оценки скорости инференса VLM-моделей. Также требуются инструменты для работы с контейнерами, поддерживаемые BenchmarkOrchestrator: Docker для одиночных серверов с несколькими GPU и Singularity для GPU-кластеров и корпоративных сред.

#### Масштабируемость
На текущий момент VLMHyperBench функционирует как система, распределяющая выполнение VLM-моделей в контейнерах на доступные GPU в операционной системе.

Поддерживаются оба подхода к масштабированию:

* Вертикальное масштабирование: переход на серверы с более производительными GPU, что позволяет ускорить выполнение вычислений.
* Горизонтальное масштабирование (основной подход): увеличение количества GPU, используемых для запуска контейнеров с VLM-моделями.

### 4.3. Требования к работе системы

Архитектура системы разработана таким образом, чтобы минимизировать задержки в работе VLM-моделей и обеспечить максимально объективные данные о скорости их инференса:

* Время инференса модели рассчитывается с момента передачи входных данных до получения ответа, при этом задержки, связанные с загрузкой данных, не учитываются в итоговых результатах.
* Каждая VLM-модель запускается на отдельном GPU, что гарантирует равные условия для работы всех моделей.

### 4.4. Безопасность системы

При использовании ПО Docker для запуска системы она наследует те же потенциальные уязвимости, что и сам Docker:

Docker использует единую точку входа — docker-sock, которая управляет всей системой. Пользователь, взаимодействующий с этой точкой, имеет права администратора, что создаёт риски для безопасности. В данном случае рекомендуется работать в защищённом контуре, используя VPN, что минимизирует вероятность проникновения злоумышленников в систему.


### 4.5. Безопасность данных

Open-source фреймворк для оценки VLM предоставляет пользователям возможность самостоятельно выбирать данные, на которых они хотят тестировать модели. Для заказчика в настоящее время используются синтетические данные, моделирующие документы граждан Российской Федерации. Все данные в этих документах сгенерированы случайным образом, и не содержат конфиденциальной информации.

### 4.7. Риски
При использовании ПО Docker для запуска системы она наследует те же потенциальные уязвимости, что и сам Docker.

Рекомендуется использовать Singularity, который изначально разработан для HPC-систем и учитывает лучшие практики безопасности для контейнеризации по умолчанию, не требуя прав администратора для своего использования.
